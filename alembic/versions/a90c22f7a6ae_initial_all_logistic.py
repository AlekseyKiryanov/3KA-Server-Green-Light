...
def upgrade() -> None:
    ...
    vkid = os.getenv('ADMIN_VK', '440265064')
    ...
    op.execute(
        text("""
            INSERT INTO users (id, name, login, hash_pass, vkid, rang)
            VALUES (
        ...
        """).bindparams(login=email, hash_pass=hash_pass, vkid=vkid)
    )
...
def downgrade() -> None:
    ...
    op.drop_table('atps')
    # ### end Alembic commands ###
    op.execute('DROP SEQUENCE atps_seq')
    op.execute('DROP SEQUENCE stops_seq')
    op.execute('DROP SEQUENCE tpus_seq')
    op.execute('DROP SEQUENCE routes_seq')
    op.execute('DROP SEQUENCE charts_seq')
    op.execute('DROP SEQUENCE timetables_seq')
    op.execute('DROP SEQUENCE traffic_seq')
