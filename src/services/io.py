...
class IOService:
    ...
    def upload_table(contents: bytes, source: str, db_session: Session, ip: str, user_id: str) -> int:
        ...
        try:
            ...
            if source == 'charts':
                df["lats"] = df["lats"].apply(
                    lambda x: [float(i) for i in ast.literal_eval(x)] if isinstance(x, str) else x)
                df["lons"] = df["lons"].apply(
                    lambda x: [float(i) for i in ast.literal_eval(x)] if isinstance(x, str) else x)
                print(type(df["lats"].iloc[0]))
                print(df["lats"].iloc[0])
            if source == 'timetables':
                df["start"] = pd.to_datetime(df["start"], format="%H:%M:%S").dt.time
                df["lap"] = pd.to_datetime(df["lap"], format="%H:%M:%S").dt.time
            ...
            if source == 'charts':
                for _, row in df.iterrows():
                    chart = Chart(id=row["id"], label=row["label"], lats=row["lats"], lons=row["lons"])
